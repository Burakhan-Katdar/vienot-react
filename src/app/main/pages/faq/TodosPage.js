...
function TodosPage() {
	const { t } = useTranslation('todosApp');
			...
			<div className="FaqPage-header flex flex-col shrink-0 items-center justify-center text-center p-16 sm:p-24 h-200 sm:h-360">
				<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1, transition: { delay: 0.3 } }}>
					<Typography
						// variant="subtitle1"
						color="inherit"
						className="opacity-75 sm:text-18 mt-8 sm:mt-16 mx-auto max-w-512 font-medium"
					>
						{t('TODOS')}
					</Typography>
				</motion.div>
				...
				<motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0, transition: { delay: 0.1 } }}>
					<Typography color="inherit" className="text-32 sm:text-36 font-bold tracking-tight">
						{t('TODO_DESCRIPTION')}
					</Typography>
				</motion.div>
			</div>
							...
							>
								{todos.map(todo => (
									<Accordion
										...
										onChange={toggleAccordion(todo.id)}
									>
