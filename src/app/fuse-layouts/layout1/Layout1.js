...
function Layout1(props) {
  ...
  useEffect(() => {
    Api.getDefaultTexts().then((response) => {
      setDefaultTexts(response.data.data)
    })
  }, [])
  return (
    <Root id="fuse-layout" config={config} className="w-full flex">
      ...
      <div className="flex flex-auto min-w-0">
        ...
        <main id="fuse-main" className="flex flex-col flex-auto min-h-screen min-w-0 relative z-10">
          ...
          {config.footer.display && defaultTexts && (
            <FooterLayout1 defaultTexts={defaultTexts} className={config.footer.style === 'fixed' && 'sticky bottom-0'} />
          )}
        </main>
